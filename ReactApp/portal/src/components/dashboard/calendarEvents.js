...
class EventComponent extends PureComponent {
  ...
  renderEvent(event) {
    ...
    switch (eventType) {
      ...
      case 'Full Day': return (<div className={`fullday ${isApproved}`} title='Leave' >

        <i className="fas fa-umbrella-beach" ></i>&nbsp;{name}
        {this.props.userRole === 'Admin' && event.IsApproved === 0 &&
          <a href='javascript:void(0)' onClick={(e) => this.onClickdialog(e, event.LeaveId, event.EmployeeId, event.EventDate)} className='rotatepoint' >...</a>}

        <i style={{ position: 'absolute' }} >  {
          this.state.showMenu ? (
            <div>
              <div className='showmenu' >
                {<a href='javascript:void(0)' onClick={this.approveLeave} style={{ color: '#000000', fontStyle: 'initial' }} >Approve</a>}<br></br>
                <a href='javascript:void(0)' style={{ color: '#000000', fontStyle: 'initial' }} onClick={this.deleteLeave} >Delete</a>
                <hr style={{ height: '2px', marginTop: '4px', marginBottom: '4px' }}></hr>
                <a href='javascript:void(0)' style={{ color: '#000000', fontStyle: 'initial' }} onClick={this.toggleModal}>Cancel</a>
              </div>
            </div>
          )
            : (
      ...
      case 'Half Day':
        return (
          <div className={`halfday ${isApproved}`} title='Leave'>
            <i className="fas fa-umbrella-beach"></i>&nbsp;{name}
            {this.props.userRole == 'Admin' && event.IsApproved === 0 &&
              <a href='javascript:void(0)' onClick={(e) => this.onClickdialog(e, event.LeaveId, event.EmployeeId, event.EventDate)} className='rotatepoint'  >...</a>}
            <i style={{ position: 'absolute' }}>  {
