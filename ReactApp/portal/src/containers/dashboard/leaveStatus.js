...
class LeaveStatus extends PureComponent {
    ...
    changeId = async () => {
        if (this.state.employeeId === '') {
            if (this.props.empId.employeeId === '') {
                await this.setState({
                ...
                await this.setState({
                    employeeId: this.props.empId.employeeId,
                    years: this.props.empId.year
                })
                this.props.employeeLeaveCount({ employeeId: this.props.empId.employeeId });
                this.props.getLeaveData({ employeeId: this.props.empId.employeeId, year: this.props.empId.year });
            }
        }
    }
    render() {
        ...
        const leaveInfo = this.props.leaveinfo;
        let _this = this;
                        ...
                        <div>
                            <div className='row' >
                                ...
                                <div className="col-md-4" style={{ marginBottom: '7px' }} ><select id='getempid' value={this.state.employeeId} onChange={this.changeEmployee} className="form-control" >
                                    {leaveInfo.getEmployeeData.Table.map(function (item, index) {
                                        ...
                                        return <tr key={index}>
                                            ...
                                            {userRole === 'Admin' &&
                                                <td style={{ width: '157px' }}>{
                                                    ...
                                                    {
                                                        <a href='javascript:void(0)' onClick={() => _this.deleteEmployeeLeave(item.LeaveId, item.EmployeeId)} title='Delete Leave' ><i className="fa fa-trash"></i></a>
                                                    }
                                                    {<i className="check-space" >{" "}</i>}
                                                    {
                                                        <a href='javascript:void(0)' onClick={() => { _this.editEmpLeave(item.EmployeeId, item.LeaveDate) }}><i title='edit Leave' className='far fa-edit'></i></a>
                                                    }
                                                </td>
                                            }
                                            {userRole !== 'Admin' &&
                                                <td style={{ width: '157px' }}>
                                                    ...
                                                    {
                                                        (item.IsApproved === false) && <a href='javascript:void(0)' onClick={() => _this.deleteEmployeeLeave(item.LeaveId, item.EmployeeId)} title='Delete Leave' ><i className="fa fa-trash"></i></a>
                                                    }
                                                    {<i className="check-space" >{" "}</i>}
                                                    {
                                                    ...
                                                    {
                                                        (item.IsApproved === false) && <a href='javascript:void(0)' onClick={() => { _this.editEmpLeave(item.EmployeeId, item.LeaveDate) }} ><i title='edit Leave' className='far fa-edit'></i></a>
                                                    }
                                                </td>
                                            }
                                        </tr>
